			...
			<el-form class="center-form-pv" :style='{"padding":"20px 10px","margin":"0 10px 20px 10px","borderRadius":"20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0","alignItems":"center","flexWrap":"wrap","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#9E9E9E","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"400","height":"40px"}' class="item-label">待办名称</label>
						<el-input v-model="searchForm.daibanmingcheng" placeholder="待办名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="是否完成" prop="shifouwancheng">
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#9E9E9E","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"400","height":"40px"}' class="item-label">是否完成</label>
						<el-select clearable v-model="searchForm.shifouwancheng" placeholder="请选择是否完成" >
							<el-option v-for="(item,index) in shifouwanchengOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('daibanxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('daibanxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"16px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="daibanmingcheng"
						label="待办名称">
						<template slot-scope="scope">
							{{scope.row.daibanmingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="daibanshijian"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('daibanxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('daibanxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('daibanxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		methods: {
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.daibanmingcheng!='' && this.searchForm.daibanmingcheng!=undefined){
					params['daibanmingcheng'] = '%' + this.searchForm.daibanmingcheng + '%'
				}
				params['sort'] = 'daibanshijian';
				params['order'] = 'asc';
				if(this.searchForm.shifouwancheng!='' && this.searchForm.shifouwancheng!=undefined){
					params['shifouwancheng'] = this.searchForm.shifouwancheng
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "daibanxinxi/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
