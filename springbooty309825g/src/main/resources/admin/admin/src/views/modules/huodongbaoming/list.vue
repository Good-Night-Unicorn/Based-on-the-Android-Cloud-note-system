<template>
	<div class="main-content" :style='{"width":"calc(100% - 35px)","margin":"10px 20px","fontSize":"16px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"20px 10px","margin":"0 10px 20px 10px","borderRadius":"20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0","alignItems":"center","flexWrap":"wrap","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#9E9E9E","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"400","height":"40px"}' class="item-label">活动名称</label>
						<el-input v-model="searchForm.huodongmingcheng" placeholder="活动名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#9E9E9E","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"400","height":"40px"}' class="item-label">活动类型</label>
				...
				<el-row class="actions" :style='{"padding":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('huodongbaoming','活动热度统计')" type="success" @click="chartDialog1()">
		...
		<el-dialog
			...
			width="800">
			<div id="huodongleixingChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var huodongleixingChart1 = echarts.init(document.getElementById("huodongleixingChart1"),'macarons');
					this.$http({
						url: "huodongbaoming/group/huodongleixing",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							huodongleixingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								huodongleixingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
									...
									if(this.shBatchForm.sfsh!='待审核'){
										for(let x in this.shList){
											await this.$http.get('yonghu/query',{params: {
												yonghuming: this.shList[x].yonghuming
											}}).then(async rs=>{
												let param = {
													...
													content: `你提交的“活动报名”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
