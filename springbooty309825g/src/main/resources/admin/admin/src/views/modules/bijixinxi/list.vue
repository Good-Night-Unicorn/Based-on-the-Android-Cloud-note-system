<template>
	<div class="main-content" :style='{"width":"calc(100% - 35px)","margin":"10px 20px","fontSize":"16px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"20px 10px","margin":"0 10px 20px 10px","borderRadius":"20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0","alignItems":"center","flexWrap":"wrap","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#9E9E9E","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"400","height":"40px"}' class="item-label">笔记名称</label>
						<el-input v-model="searchForm.bijimingcheng" placeholder="笔记名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('bijixinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('bijixinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu15" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						批量删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('bijixinxi','用户活跃度统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						用户活跃度统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('bijixinxi','笔记类型占比')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						笔记类型占比
					</el-button>
					<el-button class="btn18" v-if="isAuth('bijixinxi','用户发布量统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						用户发布量统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('bijixinxi','每日发布量统计')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						每日发布量统计
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"16px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('bijixinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('bijixinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('bijixinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('bijixinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<el-dialog
			...
			width="800">
			<div id="biaoqianChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="xingmingChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="fabushijianChart4" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		methods: {
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var biaoqianChart2 = echarts.init(document.getElementById("biaoqianChart2"),'macarons');
					this.$http({
						url: "bijixinxi/group/biaoqian",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							biaoqianChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								biaoqianChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog3() {
				...
				this.$nextTick(()=>{
					...
					var xingmingChart3 = echarts.init(document.getElementById("xingmingChart3"),'macarons');
					this.$http({
						url: "bijixinxi/group/xingming",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							xingmingChart3.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								xingmingChart3.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog4() {
				...
				this.$nextTick(()=>{
					...
					var fabushijianChart4 = echarts.init(document.getElementById("fabushijianChart4"),'macarons');
					this.$http({
						url: "bijixinxi/group/fabushijian",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							fabushijianChart4.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								fabushijianChart4.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussbijixinxi',query:{refid:id}});
			},
			// 下载
			download(file){
