		...
		<view class="container" :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#F4F4F4","height":"auto"}'>
			<view :style='{"width":"100%","padding":"24rpx","background":"none","height":"auto"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"10rpx 24rpx","flexWrap":"wrap","background":"#FFFFFF","display":"flex","gap":"20rpx","width":"100%","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","background":"linear-gradient( 90deg, #2BC8BA 0%, #459FEF 100%)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('sensitivewords','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","background":"linear-gradient( 90deg, #2BC8BA 0%, #459FEF 100%)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('sensitivewords','私聊')" @tap="chatClick">联系TA</button>
	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('sensitivewords', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','sensitivewords');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','sensitivewords');
			...
			async init(type=1){
				...
				let res = await this.$api.info('sensitivewords', this.id);
				let reg=new RegExp('http://localhost:8080/springbooty309825g/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('sensitivewords',this.detail)
				}
			},
