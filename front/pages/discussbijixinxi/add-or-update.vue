	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussbijixinxi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussbijixinxi`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussbijixinxi`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussbijixinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussbijixinxiCleanType',true);
						if(this.cross){
